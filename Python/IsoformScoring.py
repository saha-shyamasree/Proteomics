...
def readFastaFile(fastaFile):
    ...
    for r in records:
        fastaDF=fastaDF.append({'Id':r.description,'Sequence':str(r.seq)}, ignore_index=True)
...
def trainTestModel(annot, orfs, refs, digestFile, isoDigestFile, conseqFile, isoConseqFile, peptideFile, rpeptideFile, sssFile):
    ...
    knownOrfIdsStr="|".join([re.escape(mrna)+"_" for mrna in  knownOrfIds])
    ...
    restOrfs=filterFastaDF(orfs, annot[~(annot['ORF Id'].isin(trainORFIds))]["ORF Id"])
    #sTime=time.clock()
    ...
    restOrfIds=annot[~(annot['ORF Id'].isin(trainORFIds))]["mRNA"]
    ...
    rIds=annot[~(annot['ORF Id'].isin(trainORFIds)) & (annot['Class']!="novel")]["Protein ID"].unique()
...
def scoring(vcfFile, vcfIsoFile, annot, sssFile):
    ...
    restOrfsAnnotOrg['Score']=0
    restOrfsAnnotOrg['RefScore']=0
    ...
    for i in restOrfsAnnot.index.values:
        ...
        if restOrfsAnnot.loc[i]['Class']=="known variation":
        ...
        elif restOrfsAnnot.loc[i]['Variation']=='0':
            ...
            scores=isoformScore(isoVarPeps)
            restOrfsAnnot.loc[i,'Score']=scores['orf']
            restOrfsAnnot.loc[i,'RefScore']=scores['ref']
            if scores['orf']==0 and scores['ref']==0:
            ...
            orfVars=vcf[vcf['QueryID']==restOrfsAnnot.loc[i]['mRNA']]
            varPeps=getPeptideMutation(orfVars,testSSS)
            scores1=isoformScore(varPeps)
            ##varPeps is a dictionary with two keys, 'ref' and 'orf'
            orfVars=vcfIso[vcfIso['QueryID']==restOrfsAnnot.loc[i]['mRNA']]
            isoVarPeps=getPeptideBoundary(orfVars,testSSS)
            ##isoVarPeps is a dictionary with two keys, 'ref' and 'orf'
            scores2=isoformScore(isoVarPeps)
            restOrfsAnnot.loc[i,'Score']=scores1['orf']+scores2['orf']
            restOrfsAnnot.loc[i,'RefScore']=scores2['ref']+scores2['ref']
            ...
            if scores2['orf']==0 and scores2['ref']==0:
                print("For further Investigation (Isoform variation):"+str(restOrfsAnnot.loc[i]['mRNA']))
    restOrfsAnnot.to_csv("/data/SBCS-BessantLab/shyama/Data/Bristol/Human/adenovirus/conseq/scoresv4.tsv",sep='\t')
...
def refPeptide(SSS, prtId, refVarPeps, start, end):
    if refVarPeps[refVarPeps['Identified']=='Identified'].shape[0]==0:
        ...
        seqRefVarPeps=pd.DataFrame()
        for j in range(refIdentSorted.shape[0]):
            varPeps=extractPeptide(refVarPeps, prtId, start, int(refIdentSorted.iloc[j]['Start'])-1)
            if varPeps.shape[0]>0:
                seqRefVarPeps=seqRefVarPeps.append(nonOverlappingPeptide(varPeps, start, int(refIdentSorted.iloc[j]['Start'])-1))
        ...
        if varPeps.shape[0]>0:
            seqRefVarPeps=seqRefVarPeps.append(nonOverlappingPeptide(varPeps, start, end))
        return seqRefVarPeps
...
def nonOverlappingPeptide(nonOverlapPeps, start, end):
    ...
    nonOverlapPeps['distFromStart']=abs(start-nonOverlapPeps['Start'])
    firstGrp=nonOverlapPeps[nonOverlapPeps['distFromStart']==min(nonOverlapPeps['distFromStart'])]
    ...
    if currPep.shape[0]>0:
        ...
        if currPep.shape[0]>1:
            print("there are 2 peptides with same start and length")
            print(currPep)
        while int(currPep.iloc[0]['Start'])<=end:
            ...
            seqPeps=seqPeps.append(currPep)
    return seqPeps
