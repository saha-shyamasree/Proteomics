...
parser.add_argument("-v", "--vcf", nargs=1, required=True, help="full path of vcf folder", metavar="PATH")

args = parser.parse_args()
#print(args)
#onlyfiles = [ f for f in os.listdir(args.blast[0]) if os.path.isfile(os.path.join(args.blast[0],f)) ]
onlyfiles=glob.glob(args.annot[0]+"/*.assemblies.fasta.transdecoder.pep_details_annotation.csv")
...
for f in onlyfiles:
    ...
    vcf=vcfVarReader(args.vcf[0]+sample+".assemblies.fasta.transdecoder.pep_pepEvd.vcf")
    ...
    isoVcf=vcfIsoReader(args.annot[0]+sample+".assemblies.fasta.transdecoder.pep_details_annotation.csv_isoform_pep.vcf")
    ...
    protDF=readFastaFile(args.fasta[0]+sample+".assemblies.fasta.transdecoder.pep.identified.fasta")
...
unqPeptideEvdMat.to_csv(args.annot[0]+"/TGEs_pepEvd.csv", sep="\t", index=False)
aggregations={
    'ALTPeptide':'sum',
    'UnqALTPeptide':'sum',
    'SALTPeptide':'sum',
...
if pepEvdSwiss.shape[0]>0:
    print("Swissprot novel Isoform"+str(pepEvdSwiss[pepEvdSwiss['IsoUnqPeptide'].str.contains('Yes')].shape[0]))

if pepEvdTrEMBL.shape[0]>0:
    print("TrEMBL novel Isoform"+str(pepEvdTrEMBL[pepEvdTrEMBL['IsoUnqPeptide'].str.contains('Yes')].shape[0]))
